...
const News = () => {
  ...
  const GetNews = async () => {
    try {
      ...
      const response = await fetch(
        "https://uzneftegaz-backend-production.up.railway.app/api/youthNews"
      );
  ...
  const handleEditClick = (news) => {
    // FORMNI TO'LDIRISH KERAK!
    setForm({
      titleUz: news.title?.uz || "",
      titleRu: news.title?.ru || "",
      titleOz: news.title?.oz || "",
      descriptionUz: news.description?.uz || "",
      descriptionRu: news.description?.ru || "",
      descriptionOz: news.description?.oz || "",
      mediaType: [], // yangi fayllar uchun
      existingMedia: news.media || [], // mavjud rasmlar
    });
    setEditingNews(news);
  ...
  const handleEditSubmit = () => {
    ...
    // Mavjud rasmlarni saqlash (URL sifatida)
    form.existingMedia?.forEach(url => {
      fd.append("existingMedia", url);
    });
  ...
  const updateNews = async (id, formData) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://uzneftegaz-backend-production.up.railway.app/api/youthNews/${id}`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,

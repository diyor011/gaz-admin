...
const Documents = () => {
  ...
  const updateDocuments = async (id, formData) => {
    try {
      ...
      try {
        const response = await fetch(`https://uzneftegaz-backend-production.up.railway.app/api/xotinQizlar/${id}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteDocuments = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://uzneftegaz-backend-production.up.railway.app/api/xotinQizlar/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
